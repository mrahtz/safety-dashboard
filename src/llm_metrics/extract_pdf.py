...
def list_pages(source: str, crops_dir: pathlib.Path, run_id: str) -> list[dict]:
    ...
    Unlike ``list_tables`` this does no table detection -- the model reads each
    page (DeepMind cards use borderless tables pdfplumber misses). Returns the
    same section-dict shape the orchestrator expects, one per page."""
    paths.ensure()
